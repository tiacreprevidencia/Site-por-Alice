var inputServicoMobile = document.getElementById("servico-mobile");
var resultadosDivMobile = document.getElementById("resultados-mobile");

inputServicoMobile.addEventListener("input", function () {
    var consulta = inputServicoMobile.value;
    ...
    if (consulta.trim() === "") {
        resultadosDivMobile.innerHTML = "";
    } else {
        exibirResultadosMobile(resultadosFiltrados);
    }
});

function exibirResultadosMobile(resultados) {
    resultadosDivMobile.innerHTML = "";
    resultadosDivMobile.classList.remove("sem-resultado");

    if (resultados.length === 0) {
        resultadosDivMobile.innerHTML = "Nenhum serviço encontrado...";
        resultadosDivMobile.classList.add("sem-resultado");
    } else {
        ...
        resultadosDivMobile.appendChild(ul);
    }
}

// APAGAR O QUE TA ESCRITO (SEARCH)
document.addEventListener("DOMContentLoaded", function () {
    const campoPesquisa = document.getElementById("servico-mobile");
    const limparCampo = document.getElementById("limpar-campo");
    const resultadosDivMobile = document.getElementById("resultados-mobile");
    ...
    function limparCampoEPesquisa() {
        ...
        campoPesquisa.focus();
        resultadosDivMobile.innerHTML = "";
        resultadosDivMobile.style.display = "none";
    ...
    campoPesquisa.addEventListener("input", function () {
        if (campoPesquisa.value !== "") {
            ...
            resultadosDivMobile.style.display = "block";
        } else {
            limparCampo.style.display = "none";
            resultadosDivMobile.style.display = "none";
        }
